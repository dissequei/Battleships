...
import ship.GameStatus;
import ship.Settings;
import shipAi.Ai;
import shipAi.AiFactory;
...
public class JFrameGameWindow
	 ...
	 */
	private JDialogConfig oOknoUstawienia;
	/**
	 ...
	 */
	private JPanelShipMarking oPanelZaznaczanieStatkow;
	/**
	 ...
	 */
	private JComponentStatus oComponentStatusGry;
	/**
	...
	private class ActionNowaGra
		...
		public ActionNowaGra()
			{
			putValue(Action.NAME, JFrameGameWindow.LANG.getProperty("action.newGame"));
			putValue(Action.SHORT_DESCRIPTION, JFrameGameWindow.LANG.getProperty("action.newGame.desc"));
			}
	...
	private class ActionUstawienia
		...
		public ActionUstawienia()
			{
			putValue(Action.NAME, JFrameGameWindow.LANG.getProperty("action.settings"));
			putValue(Action.SHORT_DESCRIPTION, JFrameGameWindow.LANG.getProperty("action.settings.desc"));
			}
	...
	private class ActionZakoncz
		...
		public ActionZakoncz()
			{
			putValue(Action.NAME, JFrameGameWindow.LANG.getProperty("action.exit"));
			putValue(Action.SHORT_DESCRIPTION, JFrameGameWindow.LANG.getProperty("action.exit.desc"));
			}
		public void actionPerformed(ActionEvent event)
			{
			System.exit(0);
	...
	private class ActionAbout
		...
		public ActionAbout()
			{
			putValue(Action.NAME, JFrameGameWindow.LANG.getProperty("action.about"));
			putValue(Action.SHORT_DESCRIPTION, JFrameGameWindow.LANG.getProperty("action.about.desc"));
			}
		public void actionPerformed(ActionEvent oEvent)
			{
	...
	private class RozgrywkaMouseListener
		...
		public RozgrywkaMouseListener(Board oPlansza, JComponentBoard oCompPlansza)
			{
		...
		@Override public void mousePressed(MouseEvent event)
			...
			try
				...
				if (bKolejGracza == true
					...
					&& (oPlansza.getPole(oKliknietePole.getX(), oKliknietePole.getY()) == FieldTypeBoard.PLANSZA_POLE_PUSTE
						|| oPlansza.getPole(oKliknietePole.getX(), oKliknietePole.getY()) == FieldTypeBoard.PLANSZA_STATEK
						)
					)
					{
					bKolejGracza = false;
					...
					if (bTrafienie == true && oStatkiKomputer.getIloscStatkow() == oStatkiKomputer.getIloscZatopionychStatkow())
						...
						JOptionPane.showMessageDialog(JFrameGameWindow.this, LANG.getProperty("message.win"));
						return;
						}
		...
		public void actionPerformed(ActionEvent oEvent)
			...
			if (bTrafienie == true && oStatkiGracz.getIloscStatkow() == oStatkiGracz.getIloscZatopionychStatkow())
				...
				JOptionPane.showMessageDialog(JFrameGameWindow.this, LANG.getProperty("message.lose"));
				return;
				}
			else if (bTrafienie == true)
				{
	 ...
	 */
	public JFrameGameWindow(GameStatus oStatusGry, Settings oUstawienia)
		{
	 ...
	 */
	public JFrameGameWindow(GameStatus oStatusGry, Settings oUstawienia, int iSzerokosc, int iWysokosc)
		{
		...
		setTitle(JFrameGameWindow.LANG.getProperty("mainWindow.title"));
		//setSize(iSzerokosc, iWysokosc);
                setExtendedState(JFrameGameWindow.MAXIMIZED_BOTH);
                setUndecorated(true);
		...
		oOknoUstawienia = new JDialogConfig(this, oUstawienia);
		oOknoUstawienia.setLocationRelativeTo(this);
		oOknoAbout = new JDialogAbout(this, JFrameGameWindow.LANG.getProperty("mainWindow.title"));
		if (sWersja != null)
			{
		...
		oPanelZaznaczanieStatkow = new JPanelShipMarking(oUstawienia, this);
		...
		oComponentStatusGry = new JComponentStatus(this.oStatusGry);
		...
		//menu "statki"
		JMenu oMenuStatki = new JMenu(JFrameGameWindow.LANG.getProperty("menu.game"));
		...
		//menu "opcje"
		JMenu oMenuOpcje = new JMenu(JFrameGameWindow.LANG.getProperty("menu.options"));
		JMenuItem oMenuOpcjeConf = new JMenuItem(oActionUstawienia);
		oMenuOpcje.add(oMenuOpcjeConf);
		oMenu.add(oMenuOpcje);
		
		//menu "help"
		JMenu oMenuPomoc = new JMenu(JFrameGameWindow.LANG.getProperty("menu.help"));
	...
	public void dodajPlansze(Board oPlansza, boolean bWyswietlStatki, RozgrywkaMouseListener oMouseListener)
		{
		JComponentBoard oCompPlansza = new JComponentBoard(oPlansza);
	...
	public void rozpocznijRozgrywke()
		...
		oStatkiKomputer = JFrameGameWindow.generujGracza(oUstawienia);
