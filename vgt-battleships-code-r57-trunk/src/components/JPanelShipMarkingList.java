 ...
 * @version 1.0
 */
public class JPanelShipMarkingList
	extends JPanel
	{
	/**
	 ...
	 */
	public JPanelShipMarkingList(Settings oUstawienia)
		{
	...
	public void odswiez()
		...
		if (oEtykiety.size() == 0)
			{
			JLabel oEtykieta = new JLabel(JFrameGameWindow.LANG.getProperty("shipPlacement.list.header"));
		...
		for (int i = iMaxRozmiar; i >= 1; --i)
			...
			if (iIlosc > 0)
				...
				if (i == 1)
					sText = JFrameGameWindow.LANG.getProperty("shipPlacement.list.prefix") + " " + JFrameGameWindow.LANG.getProperty("shipName.size1.plural") + " (" + JFrameGameWindow.LANG.getProperty("shipPlacement.list.size") +" 1): ";
				else
					sText = JFrameGameWindow.LANG.getProperty("shipPlacement.list.prefix") + " " + JFrameGameWindow.LANG.getProperty("shipName.size" + iKlasaStatku + ".plural") + " (" + JFrameGameWindow.LANG.getProperty("shipPlacement.list.size") + " " + i + "): ";
				++iLinieTekstu;
