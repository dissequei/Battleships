 ...
 * @version 1.0
 */
public class JPanelShipSettingsList
	extends JPanel
	{
	/**
	 * Obiekt listy przechowujacy statki.
	 */
	private JListShipSettings oListaStatkow;
	/**
	...
	private class ActionDodaj
		...
		public ActionDodaj()
			{
			putValue(Action.SHORT_DESCRIPTION, JFrameGameWindow.LANG.getProperty("action.settings.shipList.add.desc"));
			URL oImgUrl = getClass().getResource("/pl/vgtworld/games/statki/img/button-add.png");
			if (oImgUrl == null)
				putValue(Action.NAME, JFrameGameWindow.LANG.getProperty("action.settings.shipList.add"));
			else
				{
	...
	private class ActionUsun
		...
		public ActionUsun()
			{
			putValue(Action.SHORT_DESCRIPTION, JFrameGameWindow.LANG.getProperty("action.settings.shipList.delete.desc"));
			URL oImgUrl = getClass().getResource("/pl/vgtworld/games/statki/img/button-delete.png");
			if (oImgUrl == null)
				putValue(Action.NAME, JFrameGameWindow.LANG.getProperty("action.settings.shipList.delete"));
			else
				{
				Image oImg = Toolkit.getDefaultToolkit().getImage(oImgUrl);
				putValue(Action.SMALL_ICON, new ImageIcon(oImg));
		...
		public void actionPerformed(ActionEvent oEvent)
			...
			if (aZaznaczone.length == 0)
				JOptionPane.showMessageDialog(JPanelShipSettingsList.this, JFrameGameWindow.LANG.getProperty("errorMsg.settings.shipList.noShipSelected"));
			try
				{
	...
	private class ActionPowieksz
		...
		public ActionPowieksz()
			{
			putValue(Action.SHORT_DESCRIPTION, JFrameGameWindow.LANG.getProperty("action.settings.shipList.increase.desc"));
			URL oImgUrl = getClass().getResource("/pl/vgtworld/games/statki/img/button-up.png");
			if (oImgUrl == null)
				putValue(Action.NAME, JFrameGameWindow.LANG.getProperty("action.settings.shipList.increase"));
			else
				{
				Image oImg = Toolkit.getDefaultToolkit().getImage(oImgUrl);
				putValue(Action.SMALL_ICON, new ImageIcon(oImg));
	...
	private class ActionPomniejsz
		...
		public ActionPomniejsz()
			...
			putValue(Action.SHORT_DESCRIPTION, JFrameGameWindow.LANG.getProperty("action.settings.shipList.decrease.desc"));
			URL oImgUrl = getClass().getResource("/pl/vgtworld/games/statki/img/button-down.png");
			if (oImgUrl == null)
				putValue(Action.NAME, JFrameGameWindow.LANG.getProperty("action.settings.shipList.decrease"));
			else
				{
				Image oImg = Toolkit.getDefaultToolkit().getImage(oImgUrl);
				putValue(Action.SMALL_ICON, new ImageIcon(oImg));
	 ...
	 */
	public JPanelShipSettingsList()
		{
		setLayout(new BorderLayout());
		
		JLabel oListaStatkowLabel = new JLabel(JFrameGameWindow.LANG.getProperty("settings.shipList.title"), JLabel.CENTER);
		oListaStatkow = new JListShipSettings();
	 ...
	 */
	public JListShipSettings getListaStatkow()
		{
