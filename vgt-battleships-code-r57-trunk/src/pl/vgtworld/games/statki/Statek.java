...
public class Statek
	 ...
	 */
	public Pozycja getPole(int iNumerPola) throws ParametrException
		{
	...
	public void setPole(int iNumerPola, int iX, int iY) throws ParametrException
		...
		if (iY + 1 > oPlansza.getWysokosc() || iY < -1)
			throw new ParametrException("iY = " + iY);
		if (iX >= 0 && iY >= 0 && oPlansza.getPole(iX, iY) != PlanszaTypPola.PLANSZA_POLE_PUSTE)
		...
		if (aWspolrzedne[ iNumerPola - 1].getX() == -1 && aWspolrzedne[ iNumerPola - 1 ].getY() == -1)
			...
			if (iX >= 0 && iY >= 0)
				...
				oPlansza.setPole(iX, iY, PlanszaTypPola.PLANSZA_STATEK);
				}
			}
		else
			{
			...
			oPlansza.setPole(aWspolrzedne[ iNumerPola - 1 ].getX(), aWspolrzedne[ iNumerPola - 1 ].getY(), PlanszaTypPola.PLANSZA_POLE_PUSTE);
							...
							for (int k = -1; k <= 1; ++k)
								for (int l = -1; l <= 1; ++l)
									...
									if (oSasiedniePole.getX() < 0 || oSasiedniePole.getX() >= oPlansza.getSzerokosc()
										...
										continue;
									if (oPlansza.getPole(oSasiedniePole.getX(), oSasiedniePole.getY()) == PlanszaTypPola.PLANSZA_POLE_PUSTE)
										oPlansza.setPole(oSasiedniePole.getX(), oSasiedniePole.getY(), PlanszaTypPola.PLANSZA_POLE_NIEDOSTEPNE);
									}
							}
						}
					return true;
					}
			oPlansza.setPole(iX, iY, PlanszaTypPola.PLANSZA_STRZAL_NIECELNY);
			return false;
			}
		catch (ParametrException e)
			{
