...
public class StatekIterator
	 ...
	 */
	private Pozycja oOstatniStrzal;
	/**
	 * Konstruktor domyslny.
	 * 
	 * @param oPlansza Referencja do obiektu planszy, na ktorej umieszczone zostana statki.
	 */
	public StatekIterator(Plansza oPlansza)
		{
		this.oPlansza = oPlansza;
		aStatki = new Statek[0];
		iIloscStatkow = 0;
		oOstatniStrzal = new Pozycja(2);
	 ...
	 */
	public Statek getStatek(int iNumer) throws ParametrException
		{
	 ...
	 */
	public Pozycja getPole(int iNrStatku, int iNrPola) throws ParametrException
		{
	...
	public boolean strzal(int iX, int iY) throws ParametrException
		...
		if (iY >= oPlansza.getWysokosc() || iY < 0)
			throw new ParametrException("iY = " + iY);
		if (oPlansza.getPole(iX, iY) != PlanszaTypPola.PLANSZA_POLE_PUSTE && oPlansza.getPole(iX, iY) != PlanszaTypPola.PLANSZA_STATEK)
	...
	public void dodajStatek(int iRozmiar)
		...
		Statek[] aNoweStatki = new Statek[ iIloscStatkow + 1 ];
		for (int i = 0; i < iIloscStatkow; ++i)
			aNoweStatki[i] = aStatki[i];
		//utworzenie nowego statku
		Statek oObj = new Statek(iRozmiar, oPlansza);
	...
	public boolean weryfikujRozmieszczenie(boolean bProsteLinie)
		...
		try
			...
			for (int iNrStatku = 1; iNrStatku <= iIloscStatkow; ++iNrStatku)
				{
				Statek oStatek = getStatek(iNrStatku);
				StatekWeryfikacja oWeryfikator = new StatekWeryfikacja();
