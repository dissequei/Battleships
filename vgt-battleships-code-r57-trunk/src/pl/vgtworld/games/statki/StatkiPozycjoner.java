...
public class StatkiPozycjoner
	 ...
	 * @deprecated
	 */
	public boolean rozmiescStatki(StatekIterator oStatki) throws ParametrException
		{
	 ...
	 */
	public boolean rozmiescStatki(StatekIterator oStatki, boolean bProsteLinie) throws ParametrException
		{
	 ...
	 */
	private void importujStatki(StatekIterator oStatki)
		{
	 ...
	 */
	private void umiescStatekNaPlanszy(Statek oStatek, boolean bProsteLinie) throws StatkiPozycjonerException
		{
		try
			{
			for (int i = 1; i <= oStatek.getRozmiar(); ++i)
				...
				if (i == 1)
					{
					//pierwsze pole
					Pozycja oPustePole = wylosujPustePole();
					...
					ArrayList<Pozycja> oKandydaci = new ArrayList<Pozycja>();
					for (int j = 1; j < i; ++j)
						{
						Pozycja oPole = oStatek.getPole(j);
						for (int k = -1; k <= 1; ++k)
							for (int l = -1; l <= 1; ++l)
								if (k + l == -1 || k + l == 1)
									if (oPole.getX() + k >= 0 && oPole.getX() + k < oPlansza.getSzerokosc()
										&& oPole.getY() + l >= 0 && oPole.getY() + l < oPlansza.getWysokosc()
										&& oPlansza.getPole(oPole.getX() + k, oPole.getY() + l) == PlanszaTypPola.PLANSZA_POLE_PUSTE
										)
										{
										Pozycja oKandydat = new Pozycja(2);
					...
					if (bProsteLinie == true && i > 2)
						...
						if (oStatek.getPole(1).getX() == oStatek.getPole(2).getX())
							...
							throw new ProgramistaException("nie mozna ustalic kierunku statku");
						}
	 ...
	 */
	private Pozycja wylosujPustePole() throws StatkiPozycjonerException
		{
		try
			{
			Pozycja oWspolrzedne = new Pozycja(2);
			//policzenie pustych pol na planszy
			int iIloscPustych = 0;
			for (int i = 0; i < oPlansza.getSzerokosc(); ++i)
				for (int j = 0; j < oPlansza.getWysokosc(); ++j)
					if (oPlansza.getPole(i, j) == PlanszaTypPola.PLANSZA_POLE_PUSTE)
						{
						Pozycja oKandydat = new Pozycja(2);
						oKandydat.setX(i);
						oKandydat.setY(j);
			...
			throw new ProgramistaException("koniec planszy");
			}
		catch (ParametrException e)
			{
			throw new ProgramistaException(e);
	 ...
	 */
	private boolean weryfikujKandydata(Pozycja oPole, Statek oStatek)
		{
		try
			{
			for (int i = -1; i <= 1; ++i)
				for (int j = -1; j <= 1; ++j)
					if (oPole.getX() + i >= 0 && oPole.getX() + i < oPlansza.getSzerokosc()
						...
						&& oPlansza.getPole(oPole.getX() + i, oPole.getY() + j) == PlanszaTypPola.PLANSZA_STATEK
						)
						{
						if (oStatek == null)
							return false;
