				...
				for (int i = -1; i <= 1; ++i)
					for (int j = -1; j <= 1; ++j)
						if (
							...
							{
							if (oStatkiPrzeciwnika.getPlansza().getPole(oWybranePole.getX() + i, oWybranePole.getY() + j) == FieldTypeBoard.PLANSZA_POLE_PUSTE
								|| oStatkiPrzeciwnika.getPlansza().getPole(oWybranePole.getX() + i, oWybranePole.getY() + j) == FieldTypeBoard.PLANSZA_STATEK
								)
								{
					...
					for (int i = -1; i <= 1; ++i)
						for (int j = -1; j <= 1; ++j)
							if (
								...
								{
								if (oStatkiPrzeciwnika.getPlansza().getPole(oWybranePole.getX() + i, oWybranePole.getY() + j) == FieldTypeBoard.PLANSZA_STRZAL_CELNY)
									{
									if (i == 0)
										bPionowy = true;
									if (j == 0)
	...
	protected boolean strzalWielokrotny(ShipIterator oStatkiPrzeciwnika, int iIloscPowtorzen)
		...
		try
			...
			for (int i = 1; i <= iIloscPowtorzen; ++i)
				...
				if (oPlansza.getPole(oWylosowanePole.getX(), oWylosowanePole.getY()) == FieldTypeBoard.PLANSZA_STATEK || i == iIloscPowtorzen)
					{
					boolean bStrzal;
	...
	private Position losujPole(Board oPlanszaPrzeciwnika)
		...
		try
			...
			while (iWylosowanePole > 0)
				{
				if (oPlanszaPrzeciwnika.getPole(iX, iY) == FieldTypeBoard.PLANSZA_POLE_PUSTE || oPlanszaPrzeciwnika.getPole(iX, iY) == FieldTypeBoard.PLANSZA_STATEK)
					--iWylosowanePole;
