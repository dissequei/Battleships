...
public class ShipGenerator
	...
	private void znajdzPola()
		...
		try
			...
			for (int i = 0; i < aPolaStatkow.length; ++i)
				oPlansza.setPole(aPolaStatkow[i].getX(), aPolaStatkow[i].getY(), FieldTypeBoard.PLANSZA_POLE_PUSTE);
			}
		catch (ParametrException e)
			{
			throw new DevException(e);
	...
	private Position[] generujStatek()
		...
		if (aPolaStatkow.length == 0)
			throw new DevException("Brak pol na liscie");
		try
			{
	 ...
	 */
	private Position pobierzPole(int iIndex) throws ParametrException
		{
